...
model = dict(
    ...
    backbone=dict(
        ...
        cls_mode=False,
        DaBN_conditions=("S3DIS", "3D_Front", "S3DIS_3D_Front"),
        DaBN_update="zero",
    ),
    criteria=[
    ...
    source_index=(
        [19, 1, 0, 30, 31, 8, 7, 6, 4, 5, 9, 29],
        [i for i in range(36, 36 + 71)]
    ),
    common_index=(
        [1, 0, 8, 7, 6, 4, 5, 9],
        [0, 1, 4, 5, 6, 7, 8, 9, 13, 2, 3]
    ),
)
ignore_index=-1
# scheduler settings
...
s3_pretransform = [
    dict(type="CenterShift", apply_z=True),
    dict(type="RandomDropout", dropout_ratio=0.5, dropout_application_ratio=0.5),
    dict(type="SphereCrop", point_max=360000, mode="random"),
    dict(type="RandomRotate", angle=[-1, 1], axis="z", center=[0, 0, 0], p=0.5),
    dict(type="RandomRotate", angle=[-1 / 64, 1 / 64], axis="x", p=0.5),
    dict(type="RandomRotate", angle=[-1 / 64, 1 / 64], axis="y", p=0.5),
    dict(type="RandomScale", scale=[0.9, 1.1]),
...
s3_pretransform_mix = s3_pretransform + [dict(type="SphereCrop", point_max=204800, mode="random"),]
# dataset settings
ignore_index = -1
...
common_map_ft = {
    ...
    37: 0, # Wall Inner
    38: ignore_index, # Customized Ceiling
    39: 9, # Cabinet
    40: ignore_index, # Light Band
    41: ignore_index, # Smart Customized Ceiling
    42: 1, # Floor
    ...
    67: ignore_index, # Ceiling
    68: 0, # Customized Feature Wall
    69: ignore_index, # Extrusion Customized Ceiling Model
    70: 0, # Extrusion Customized Background Wall
}

ft_common_names = [
    "wall", "floor", "chair", "sofa", "table",
    "door", "window", "bookcase", "desk", "cabinet",
    "bed", 
]
...
source_map_s3 = {
    ...
    10: 10, # "bookcase",
    11: 11, # "board",
    12: ignore_index, # "clutter",
}

common_map_s3 = {
    -1: ignore_index,
    0: ignore_index, # "ceiling",
    1: 0, # "floor",
    2: 1, # "wall",
    3: ignore_index, # "beam",
    4: ignore_index, # "column",
    5: 2, # "window",
    6: 3, # "door",
    7: 4, # "table",
    8: 5, # "chair",
    9: 6, # "sofa",
    10: 7, # "bookcase",
    11: ignore_index, # "board",
    12: ignore_index, # "clutter",
}

s3_common_names = [
    "floor", "wall", "window", "door", "table",
    "chair", "sofa", "bookcase",
]
...
data = dict(
    num_classes=[8, 11, 11],
    ignore_index=-1,
    names=[s3_common_names, ft_common_names, sc_names],
    train=dict(
        type="ConcatDataset",
        datasets=[
            dict(
                type="MergeDataset",
                split=[["Area_1", "Area_2", "Area_3", "Area_4", "Area_6"], "train"],
                data_root=["data/s3dis", "data/3d_front/train"],
                data_name=["S3DIS", "3D_Front"],
                pre_transform=[
                    s3_pretransform, ft_pretransform
                ],
                ignore_index=-1,
                dataset_idx=1,
                condition_idx=1,
                ...
                source_mapping=[source_map_s3, source_map_ft],
                common_mapping=[common_map_s3, common_map_ft],
                transform=[
                    dict(
                        type="GridSample",
                        grid_size=0.05,
                        ...
                        mode="train",
                        return_grid_coord=True,
                        keys=("coord", "color", "segment", "condition", "instance", "dataset_idx"),
                    ),
                    dict(type="SphereCrop", point_max=204800, mode="random"),
                    dict(type="ToTensor"),
                    dict(
                        type="Collect",
                        keys=("coord", "grid_coord", "segment", "condition", "instance", "dataset_idx"),
                        feat_keys=("color",),
                    ),
                ],
            ),
    ...
    val=dict(
        ...
        datasets = [
            dict(
                type="MergeDataset",
                split=[["Area_5", ], "train"],
                data_root=["data/s3dis", "data/3d_front/val"],
                data_name=["S3DIS", "3D_Front"],
                pre_transform=[
                    [dict(type="CenterShift", apply_z=True),], 
                    [dict(type="CenterShift", apply_z=True),],
                ...
                mix_same_domain=False,
                mix_cross_domain=False,
                source_mapping=[common_map_s3, common_map_ft],
                common_mapping=[common_map_s3, common_map_ft],
                transform=[
                    dict(
                        type="GridSample",
    ...
    test=dict(
        ...
        datasets = [
            dict(
                type="MergeDataset",
                split=["val", "train", ["Area_1", "Area_2", "Area_3", "Area_4", "Area_6"]],
                data_root=["data/scannet", "data/3d_front/val", "data/s3dis"],
                data_name=["ScanNet", "3D_Front", "S3DIS"],
                pre_transform=[
                    [dict(type="CenterShift", apply_z=True),], 
                    [dict(type="CenterShift", apply_z=True),],
                    [dict(type="CenterShift", apply_z=True),],
                ...
                mix_dataset_idx=[0,],
                mix_dataset_prob=0.0,
                data_type=["Indoor", "Indoor", "Indoor"],
                loop=1,
                ratio=1,
                skip=[1,1,1],
                mix_prob=0.0,
                ...
                mix_same_domain=False,
                mix_cross_domain=False,
                source_mapping=[common_map_sc, common_map_ft, common_map_s3],
                common_mapping=[common_map_sc, common_map_ft, common_map_s3],
