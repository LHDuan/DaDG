...
class RandomDropout(object):
    ...
    def __call__(self, data_dict):
        ...
        if random.random() < self.dropout_application_ratio and n > 102400:
            ...
            if "condition" in data_dict.keys():
                data_dict["condition"] = data_dict["condition"][idx]
            if "dataset_idx" in data_dict.keys():
                data_dict["dataset_idx"] = data_dict["dataset_idx"][idx]
        return data_dict
...
@TRANSFORMS.register_module()
class RandomFeatureDrop(object):
    def __init__(self, feature_augment=0.0, p=0.00, feat_keys=["color"]):
    ...
    def __call__(self, data_dict):
        for feat_key in self.feat_keys:
            if feat_key in data_dict.keys() and np.random.rand() < self.p:
                data_dict[feat_key] = data_dict[feat_key] * self.feature_augment
        return data_dict
...
class SphereCrop(object):
    ...
    def __call__(self, data_dict):
        ...
        # mode is "random" or "center"
            ...
            if "condition" in data_dict.keys():
                data_dict["condition"] = data_dict["condition"][idx_crop]
            if "dataset_idx" in data_dict.keys():
                data_dict["dataset_idx"] = data_dict["dataset_idx"][idx_crop]
        return data_dict
...
@TRANSFORMS.register_module()
class RandomBeamDropout(object):
    def __init__(self, beam_sampling_ratio=[0.4, 0.6], dropout_application_ratio=0.5, beam_num=64, new_condition=0):
        """
        upright_axis: axis index among x,y,z, i.e. 2 for z
        """
        self.beam_sampling_ratio = beam_sampling_ratio
        self.dropout_application_ratio = dropout_application_ratio
    ...
    def __call__(self, data_dict):
        if random.random() < self.dropout_application_ratio:
            ...
            fov_down = np.percentile(pitch, 1.0)
            fov_up = np.percentile(pitch, 99.0)
            fov = fov_up - fov_down
            ...
            min_beam_num = int(self.beam_num*self.beam_sampling_ratio[0])
            max_beam_num = int(self.beam_num*self.beam_sampling_ratio[1])

            final_beam_num = random.randint(min_beam_num, max_beam_num)
            
            # randomly choose beams to remain
            remain_beam_list = random.sample(range(0, self.beam_num), final_beam_num)
            idx = np.zeros((proj_y.shape[0])).astype(np.bool_)
            ...
            if "dataset_idx" in data_dict.keys():
                data_dict["dataset_idx"] = data_dict["dataset_idx"][idx]
                # if self.new_condition > 0:
                #     data_dict["condition"] += self.new_condition
        return data_dict
