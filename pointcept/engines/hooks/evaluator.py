...
@HOOKS.register_module()
class SemSegEvaluatorMultiDataset(HookBase):
    def __init__(self):
    ...
    def eval(self):
        self.trainer.model.eval()
        m_iou_sum = 0
        for val_loader_ind in range(len(self.trainer.val_loader)):
            self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
            torch.cuda.empty_cache()
            val_loader = self.trainer.val_loader[val_loader_ind]
            for i, input_dict in enumerate(val_loader):
                ...
                intersection, union, target = intersection_and_union_gpu(
                    pred,
                    segment,
                    self.trainer.cfg.data.num_classes[val_loader_ind],
                    self.trainer.cfg.data.ignore_index,
                )
                if comm.get_world_size() > 1:
                    dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(
                ...
                self.trainer.storage.put_scalar("val_loss", loss.item())
                info = "Test: [{iter}/{max_iter}] ".format(
                    iter=i + 1, max_iter=len(val_loader)
                )
                if "origin_coord" in input_dict.keys():
                    info = "Interp. " + info
                if (i + 1) % 100 ==0:
            ...
            for i in range(self.trainer.cfg.data.num_classes[val_loader_ind]):
                self.trainer.logger.info(
                    "Class_{idx}-{name} Result: iou/accuracy {iou:.4f}/{accuracy:.4f}".format(
                        idx=i,
                        name=self.trainer.cfg.data.names[val_loader_ind][i],
                        iou=iou_class[i],
                        accuracy=acc_class[i],
                    )
                )
            ...
            if self.trainer.writer is not None:
                ...
                self.trainer.writer.add_scalar("val/mAcc", m_acc, current_epoch)
                self.trainer.writer.add_scalar("val/allAcc", all_acc, current_epoch)
            self.trainer.storage.history("val_loss").reset()
            self.trainer.storage.history("val_intersection").reset()
            self.trainer.storage.history("val_union").reset()
            self.trainer.storage.history("val_target").reset()
            m_iou_sum += m_iou
            self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
            torch.cuda.empty_cache()
        self.trainer.comm_info["current_metric_value"] = m_iou_sum  # save for saver
        self.trainer.comm_info["current_metric_name"] = "mIoU"  # save for saver
